...
export default function ProfileScreen() {
  ...
  const [role, setRole] = useState<string | null>(null);
  const [editModal, setEditModal] = useState(false);
  const [verifyModal, setVerifyModal] = useState(false); // ✅ Modal për verify
  ...
  const resendVerification = async () => {
    try {
      ...
      setVerifyModal(true); // ✅ Hap modal për të treguar mesazhin
    } catch (err: any) {
      console.log(err.response?.data || err.message);
      setResendMsg("Could not resend verification");
      setVerifyModal(true);
    } finally {
